...
#### [가이드라인](../../undefined.md#id-1-1.5-twap)
...
>   * Compound, [Synthetix](../../undefined.md#synthetix-compound-synthetix-1-3) 등의 DeFi 프로토콜은 1% 이내의 오라클 괴리를 허용 한계로 지정하고 괴리 누적에 의한 유동성 공급자 손실 방지를 위해 3분 이상 갱신되지 않으면 거래 일시 정지\
...
>   * 연산 중간값을 고정소수점 단위로 변환 후 사용하여 중간 [계산 결과](../../undefined.md#lp-18-lp-0.1)의 정밀도가 1e18 미만으로 떨어지지 않도록 검증 및 유지
...
>   *   유동성 제거 시, 최근 N 블록의 평균 가격(TWAP)을 정산 기준으로 활용하여 [일시적 가격 조작방지](../../undefined.md#twap-n-n)
>
>       $$\text{TWAP} = \frac{1}{N} \sum_{j=1}^{N} \text{Price}_{\text{block }j} \space {\scriptsize (N= \text{Block Number})}$$
>   *   Curve, Balancer 등과 같이 프로토콜 레벨에서 유동성 제공 후 LP 토큰 수령 시 [최소 보유 기간](../../undefined.md#twap-lp-mev)이 지나야만 유동성 제거가 가능하도록 조건 추가
>
...
> * **자동 리밸런싱 메커니즘**
>   *   Uniswap, Curve 등의 AMM 서비스와 같이 유동성 풀 내 자산 가치 비율 유지를 위한 목표 비율 대비 편차 [임계값 설정](../../undefined.md#undefined)하여 초과 시 리밸런싱을 트리거 하도록 실시
>
...
>   * Uniswap의 [x\*y=k ](../../undefined.md#curve-stableswap-x-y-k)곡선과 같이 편차 발생 시 스마트 컨트랙트에서 시 자동 리밸런싱하는 트리거를 제공하여 가격 균형 회복 유도
> * **불균형 모니터링**
...
>   *   사용자가 입력한 최소 수량과 실제 계산된 최소 아웃풋이 일치하는지 [검증하기 위해 수식](../../undefined.md#undefined-1)을 활용하여 실제 지급량 확인 (프로토콜에 따라 수식 종류가 다를 수 있음)
>
...
> * **대량 거래 시 분할 처리:**
>   * [1inch](../../undefined.md#id-1inch-mev) 네트워크 등의 DEX와 동일하게 여러 DEX / 유동성 풀에 대형 거래를 분할하여 슬리피지를 최소화하고 각  거래별 슬리피지 검증 실시
...
> * **자동화된 수수료 관리:**
>   *   Uniswap, Balancer 등의 DEX와 같이 일정 이상의 수수료 누적 임계값 도달 시 [자동 수집 트리거](../../undefined.md#undefined-2)되도록 프로토콜 레벨에서 처리
>
...
>
> * **권한 및 변경 관리:**
>   *   대량 인출 또는 민감한 관리자 함수 실행 시  [타임락 적용](../../undefined.md#makerdao-48)을 아래 수식과 같이 적용
>
>       $$\scriptsize \text{(Example: Execute Time} = \text{Request Time} + \text{Time} - \text{Lock Period})$$
>   * Uniswap, Curve 등과 같이 수수료 변경 시 한 번에 적용하는 것이 아닌 [단계적](../../undefined.md#undefined-3)으로 수수료 적용 (예: 0.05%) \[출처: [Uniswap Docs](https://docs.uniswap.org/concepts/protocol/fees)]
...
> * **원자적 거래 보장:**
>   * 다른 AMM 및 DEX 스마트 컨트랙트와 같이 모든 풀 상태 변경을 [단일 트랜잭션 내 처리](../../undefined.md#require-assert)하여 관련 변수를 한 번에 갱신
...
>   *   여러 풀 또는 토큰 간의 연동으로 인해 발생하는 아비트라지를 줄이기 위해 정해진 수식을 이용하여 풀 간 가격 일관성 확인 및 [총 토큰 공급량 보존 검증](../../undefined.md#balancer-weightedmath-x-y-k)
>
