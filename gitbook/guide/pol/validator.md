...
>   * timestamp를 eip-4788의 [history\_buf\_length](../../undefined.md#history_buf_length-eip-4788-8191-12-2)로 mod 연산을 하여 `_processedTimestampsBuffer`에 삽입
>   * EIP-4788의 ring buffer 메커니즘에 따라 8191 슬롯([약 4.55시간, 2초 간격 기준](../../undefined.md#id-4.55-8191-2-8)) 후 덮어씌워짐, 이는 보상 주기와 일치함&#x20;
> *   **Beacon block root과 proposer index/pubkey 의 암호학적 검증**
>
>     * &#x20;[`SSZ.verifyProof`](../../undefined.md#ssz.verifyproof-simple-serialize-merkle-proposer-index) 함수를 사용하여, 특정 타임스탬프의 비콘 루트를 기준으로 해당 제안자의 보상 자격을 검증
>     * 검증 실패시 revert 발생
>
>     ```solidity
...
> * **운영자 변경 시**[ **queue 메커니즘과 시간 지연**](../../undefined.md#beacondeposit-24)**을 통한 급작스러운 변경 방지**
...
> * **운영자 변경 시 기존 예치 잔액에 대한** [**잠금 기간 설정 및 점진적 권한**](../../undefined.md#greater-than-20) **이전**
...
[자발적 출금 로직 부재](../../undefined.md#queue-2)로 자금 동결되지만, 베라체인의 [ValidatorSetCap](../../undefined.md#validatorsetcap)에 따라 강제 퇴출 시 회수 가능하며, 이는 자산 손실보다는 네트워크 안정성에 잠재적 영향을 미쳐 `Low`로 평가한다.
