...
검증자 보상 중복 수령 및 누락은 검증자의 손해를 초래하나 Merkle root 검증 기법으로 공격 성공 가능성이 매우 낮아`Low`로 평가
...
악의적 운영자로 인한 피해가 위임 자산의 직접적인 손실보다는 평판 저하 및 미래의 BGT 위임으로 수익 감소에 국한되어 `Low`로 평가
...
자발적 출금 로직 부재로 자금이 일시적으로 동결되지만, 이는 자산의 직접적인 손실이나 탈취가 아니며 향후 검증자 자격(cap) 변동 시 회수 가능하므로 영향도를 `Low`로 평가
