...
* `디페깅 (Depegging)` : 스테이블코인이나 자산이 기준 페깅 가격에서 일정 수준 이상 벗어나는 현상
...
* `재진입 공격`: 스마트 컨트랙트의 출금 함수 등 외부로 자산을 전송한 후 내부 상태(예: 잔액)등을 업데이트하는 취약점을 악용하는 공격. 공격자는 컨트랙트가 자금을 전송하고 그 자금을 수령하는 fallback(또는 receive)함수를 통해 컨트랙트의 동일한 함수로 재진입. 내부 상태가 아직 업데이트되지 않았다는 점을 이용해 반복적으로 자금 인출 가능
* `ERC20 Compliance`: 이더리움 블록체인에서 발행되는 토큰이 ERC20 표준이 정의한 함수와 이벤트를 정확하게 구현하는 것. ERC20은 "Ethereum Request for Comments 20"의 약자로 이더리움 네트워크에서 대체 가능한 토큰(Fungible Token)의 호환성과 상호운용성을 보장하는 표준\
...
* `Merkle root 검증 기법`: Simple Serial 기반 Merkle 증명 검증 모듈인 SSZ.verifyProof 모듈을 이용하여 특정 타임스탬프의 비콘 루트를 기준으로 잘못된 proposer index 제출 시 트랜잭션이 자동으로 실패 처리되도록 설정
* `iBERA/BERA`: 베라체인의 네이티브 토큰인 BERA와 Infrared dApp의 전용 토큰인 iBERA 간 교환 풀
* `Bribe 시스템`: Infrared 유동성 풀에 더 많은 BGT 인센티브가 배분되도록 Infrared 관련 검증자에게 추가 보상을 제공해, 유동성 공급자와 검증자의 행동을 Infrared 쪽으로 유도하는 인센티브 메커니즘
* `대량 수확`: 수수료 변경 전후 시점을 노려 한 번에 많은 보상을 청구해 부당이득을 얻으려는 행위
