...
>     * **±0.1%:** Honey 페깅 허용범위(0.2%)의 50% 수준
>     * **±0.15%:** Honey 페깅 허용범위의 75% 수준
...
> * **오라클 간의 로직 비대칭성 확인**
>   * "Spot 오라클 가격이 $1.00 초과 시 → $1.00으로 처리" 같은 오라클 강제화가 아닌 일반화 로직 필요
...
민팅과 리딤 로직의 basket 모드가 각각 따로 동작한다면 혼란을 가중시킬 수 있다. 또한 세분화된 basket 모드를 통해 사용자 편의성을 올리는 편을 권고하기 위해 `Informational` 로 평가

#### 가이드라인

> * **민감도 조정 기준:** 민팅과 리딤시에 각각 따로 basket 모드가 따로 동작하는 것이 아니라 [가격 변동률](../../reference.md#id-23)<sub>23</sub> 차이별로 basket 모드의 단계를 나누어 적용
...
>   * **안정성 회복:** [1시간 연속 안정(0.2% 미만)](../../reference.md#id-24.-ispegged)<sub>24</sub> 시 자동으로 정상 모드 복귀
...
> * **Basket 모드가 활성화 된 상태에서 상환 시 디페깅된 자산은** [**3개 이상의 오라클**](../../reference.md#id-25.-chainlink)<sub>25</sub>**을 참조하여 자산의 가치를 평가(현재 베라체인은 신뢰가능한 chainlink 오라클 및 HONEY 오라클로 pyth, spot 오라클 참조하고 있음)**
...
>   * 디페깅된 자산으로 인한 예상 손실을 어떻게 계산하는지에 대한 [수식 기반 설명](../../reference.md#id-26.-calculateloss)<sub>26</sub>
> * **필요시 프로토콜 차원에서 디페깅된 자산으로 인한 급격한 손실 위험을 일부 완화할 수 있는 내부 준비금 운영 고려**
>   * 상환과정에서 발생한 수수료의 일부를 활용해서 내부 준비금으로 운영
